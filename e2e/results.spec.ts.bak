...
test.describe('Results Page', () => {
      ...
      matches: [
        {
          ...
          players: [
            { playerId: 'p1', buyIns: 1, finalValue: 1000 }, // 10.00
      ...
      matches: [
        {
          ...
          buyInAmount: 1000,
          players: [
            { playerId: 'p1', buyIns: 1, finalValue: 2000 }, // paid 10, final 20 → net +10
            { playerId: 'p2', buyIns: 1, finalValue: 0 },    // paid 10, final 0 → net -10
          ],
          startedAt: new Date().toISOString(),
          createdAt: new Date().toISOString(),
        },
  ...
  test('multi‑player transfers show minimized payments', async ({ page }) => {
      ...
      matches: [
        {
          ...
          buyInAmount: 1000,
          players: [
            { playerId: 'p1', buyIns: 2, finalValue: 3500 }, // paid 20, final 35 → net +15
            { playerId: 'p2', buyIns: 1, finalValue: 0 },    // paid 10, final 0 → net -10
            { playerId: 'p3', buyIns: 1, finalValue: 500 },  // paid 10, final 5 → net -5
          ],
          startedAt: new Date().toISOString(),
          createdAt: new Date().toISOString(),
        },
    ...
    const transfer10 = transferItems.filter({ has: page.getByText('10.00 EUR') });
  ...
  test('full flow from cashout to results', async ({ page }) => {
    ...
    const aliceRow = page.getByTestId('player-row').filter({ has: page.getByRole('heading', { name: 'Alice' }) });
    ...
    await fillCashoutValues(page, { 'Alice': 25.00, 'Bob': 5.00 });
