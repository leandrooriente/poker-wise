...
test.describe('Live Match', () => {
  ...
  test('rebuys across multiple players', async ({ page }) => {
      ...
      matches: [
        {
          ...
          players: [
            ...
            { playerId: 'p3', buyIns: 1, finalValue: 0 },
          ],
          startedAt: new Date().toISOString(),
          createdAt: new Date().toISOString(),
        },
    ...
    await expect(page.getByText('Alice').locator('..').getByText('Buy‑ins: 3')).toBeVisible();
    await expect(page.getByText('Bob').locator('..').getByText('Buy‑ins: 2')).toBeVisible();
    await expect(page.getByText('Charlie').locator('..').getByText('Buy‑ins: 1')).toBeVisible();
  ...
  test('total pot updates correctly after rebuys', async ({ page }) => {
    ...
    await expect(page.getByText('Total buy‑ins').locator('..').locator('span.font-pixel')).toHaveText('6');
  });
  ...
  test('proceed to cashout preserves state', async ({ page }) => {
    ...
    await expect(page.getByText('Alice').locator('..').getByText('Paid in: 30.00 EUR')).toBeVisible();
    await expect(page.getByText('Bob').locator('..').getByText('Paid in: 20.00 EUR')).toBeVisible();
  });
